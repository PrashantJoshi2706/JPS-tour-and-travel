...
addBtn.onclick = function () {
  ...
  fetch("http://localhost:5070/insert", {
    ...
    method: "POST",
    body: JSON.stringify({ name: name, mob: mob, eamil: email }),
  })
    .then((response) => response.json())
...
function insertRowIntoTable(data) {
  ...
  for (var key in data) {
    ...
    tableHtml += `<td><button class="delete-row-btn" data-id=${data.c_id}>DELETE</td>`;
    tableHtml += `<td><button class="edit-row-btn" data-id=${data.c_id}>edit</td>`;
  }
...
function loadHTMLTable(data) {
  ...
  let tableHtml = "";
  data.forEach(function ({ c_id, c_name, date_added, c_mobile_no, c_email }) {
    tableHtml += "<tr>";
